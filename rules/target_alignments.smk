...
rule target_alignments:
    ...
    run:
        ...
        blast_output.columns = ['qseqid', 'sseqid', 'pident', 'qcovhsp', 'length', 'mismatch', 'gapopen', 'qstart', 'qend', 'sstart', 'send', 'evalue', 'bitscore', 'staxids']
        target_alignments = blast_output.loc[blast_output.qcovhsp >= config['target_alignment_qcovhsp'],
                                         ['sseqid','sstart','send']]
